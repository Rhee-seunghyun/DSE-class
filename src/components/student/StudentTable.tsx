...
export function StudentTable({ students, onEdit, onDelete, onCheckboxChange }: StudentTableProps) {
  ...
  const renderColumnHeader = (field: keyof StudentData, label: string, sortable = true, filterable = true) => {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="min-w-[100px]">
              {renderColumnHeader('license_number', '면허번호')}
            </TableHead>
            <TableHead className="min-w-[150px]">
              {renderColumnHeader('email', '이메일')}
            </TableHead>
            <TableHead className="min-w-[120px]">
              {renderColumnHeader('phone_number', '연락처')}
            </TableHead>
            <TableHead className="w-20 text-center">
              {renderCheckboxHeader('payment_confirmed', '입금확인')}
            </TableHead>
            <TableHead className="w-24 text-center">
              {renderCheckboxHeader('business_registration', '사업자등록증')}
            </TableHead>
            <TableHead className="w-20 text-center">
              {renderCheckboxHeader('invoice_issued', '계산서발행')}
            </TableHead>
            <TableHead className="w-20 text-center">
              {renderCheckboxHeader('survey_completed', '설문조사')}
            </TableHead>
            <TableHead className="w-20 text-center">
              {renderCheckboxHeader('certificate_sent', 'Certi발송')}
            </TableHead>
          ...
          {filteredAndSortedStudents.length > 0 ? (
            filteredAndSortedStudents.map((student, index) => (
              <TableRow key={student.id}>
                ...
                <TableCell className="text-sm">{student.student_name || 'null'}</TableCell>
                <TableCell className="text-sm">{student.license_number || 'null'}</TableCell>
                <TableCell className="text-sm">{student.email || 'null'}</TableCell>
                <TableCell className="text-sm">{student.phone_number || 'null'}</TableCell>
